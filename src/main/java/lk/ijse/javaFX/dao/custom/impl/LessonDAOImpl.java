...
public class LessonDAOImpl implements LessonDAO {
    ...
    public String getLastId() throws Exception {
        Session session = factoryConfiguration.getSession();
        try {
            Query<String> query = session.createQuery("SELECT l.l_id FROM Lessons l ORDER BY l.l_id DESC", String.class)
                    .setMaxResults(1);
    ...
    public List<String> getAllIds() throws Exception {
        Session session = factoryConfiguration.getSession();
        try {
            Query<String> query = session.createQuery("SELECT l.l_id FROM Lessons l", String.class);
            return query.list();
        } finally {
            session.close();
        }
