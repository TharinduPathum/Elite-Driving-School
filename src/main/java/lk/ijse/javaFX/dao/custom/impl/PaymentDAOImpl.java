...
public class PaymentDAOImpl implements PaymentDAO {
    ...
    public String getLastId() throws Exception {
        Session session = factoryConfiguration.getSession();
        try {
            Query<String> query = session.createQuery("SELECT p.p_id FROM Payments p ORDER BY p.p_id DESC", String.class)
                    .setMaxResults(1);
    ...
    public List<String> getAllIds() throws Exception {
        Session session = factoryConfiguration.getSession();
        try {
            Query<String> query = session.createQuery("SELECT p.p_id FROM  Payments p", String.class);
            return query.list();
        } finally {
            session.close();
        }
